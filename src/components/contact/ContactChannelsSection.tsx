        ...
        {CONTACT_CHANNELS.map((ch) => {
          ...
          return (
            <article
              key={ch.id}
              data-cursor-hover
              className="group flex h-full flex-col rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1 sm:p-8"
            ...
            >
              ...
              <h2
                className="mt-6 text-xl font-bold tracking-tight sm:text-[1.35rem] transition-colors duration-300"
                style={{ color: "var(--text-primary)" }}
              >
                {ch.title}
              </h2>
              <p
                className="mt-3 flex-1 text-[0.9375rem] leading-relaxed sm:mt-4 transition-colors duration-300"
              ...
              <Link
                href={ch.href}
                className={`mt-7 inline-flex items-center gap-1.5 text-sm font-semibold transition-colors sm:mt-8 ${a.link}`}
              >
                {ch.linkLabel}
